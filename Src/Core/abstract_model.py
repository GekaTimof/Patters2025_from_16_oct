...
class abstact_model(ABC):
    ...
    def to_dto(self):
        ...
        # проверяем, что модкль может быть конвертированиа в dto
        if not common.can_convert_to_dto(self):
            raise argument_exception(f"объект {self} не имеет конвертации в dto (не установлен dto_type)")
